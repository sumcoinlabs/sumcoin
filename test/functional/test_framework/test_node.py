...
class TestNode():
    ...
    def __init__(self, i, dirname, extra_args, rpchost, timewait, binary, stderr, mocktime, coverage_dir):
        ...
        if timewait:
            ...
            self.rpc_timeout = 60
        if binary is None:
            self.binary = os.getenv("SUMCOIND", "sumcoind")
        else:
            self.binary = binary
        self.stderr = stderr
        ...
        self.cli = TestNodeCLI(os.getenv("SUMCOINCLI", "sumcoin-cli"), self.datadir)
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            assert self.process.poll() is None, "sumcoind exited with status %i during initialization" % self.process.returncode
            try:
        ...
        raise AssertionError("Unable to connect to sumcoind")
...
class TestNodeCLI():
    ...
    def __call__(self, *args, input=None):
        # TestNodeCLI is callable with sumcoin-cli command-line args
