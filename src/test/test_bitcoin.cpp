...
TestingSetup::TestingSetup(const std::string& chainName) : BasicTestingSetup(chainName)
{
    const CChainParams& chainparams = Params();
        ...
        ClearDatadirCache();
        pathTemp = fs::temp_directory_path() / strprintf("test_sumcoin_%lu_%i", (unsigned long)GetTime(), (int)(InsecureRandRange(100000)));
...
TestChain100Setup::TestChain100Setup() : TestingSetup(CBaseChainParams::REGTEST)
{
    ...
    for (int i = 0; i < Params().GetMaturity(chainActive.Height()).nCoinbaseMaturity; i++)
    {
