...
static UniValue validateaddress(const JSONRPCRequest& request)
{
    RPCHelpMan{
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The sumcoin address to validate"},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
...
static UniValue verifymessage(const JSONRPCRequest& request)
{
    RPCHelpMan{
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The sumcoin address to use for the signature."},
...
bool getIndexKey(const CTxDestination& dest, uint256& hashBytes, int& type)
{
    if (!IsValidDestination(dest)) {
        return false;
    } else if (dest.type() == typeid(PKHash)) {
        auto x = boost::get<PKHash>(&dest);
        memcpy(&hashBytes, x->begin(), 20);
        type = 1;
        return true;
    } else if (dest.type() == typeid(ScriptHash)) {
        auto x = boost::get<ScriptHash>(&dest);
        memcpy(&hashBytes, x->begin(), 20);
        type = 2;
        return true;
    } else if (dest.type() == typeid(WitnessV0ScriptHash)) {
        auto x = boost::get<WitnessV0ScriptHash>(&dest);
        memcpy(&hashBytes, x->begin(), 32);
        type = 3;
        return true;
    } else if (dest.type() == typeid(WitnessV0KeyHash)) {
        auto x = boost::get<WitnessV0KeyHash>(&dest);
        memcpy(&hashBytes, x->begin(), 20);
        type = 4;
        return true;
    }
