    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
            ...
            for (size_t j = 0; j < tx.vin.size(); j++) {
                ...
                if (GetSpentIndex(spentKey, spentInfo)) {
                    if (spentInfo.addressType == 1) {
                        delta.pushKV("address", EncodeDestination(PKHash(CKeyID(spentInfo.addressHash))));
                    } else if (spentInfo.addressType == 2) {
                        delta.pushKV("address", EncodeDestination(ScriptHash(CScriptID(spentInfo.addressHash))));
                    } else {
                        continue;
                    }
        ...
        for (unsigned int k = 0; k < tx.vout.size(); k++) {
            ...
            if (out.scriptPubKey.IsPayToScriptHash()) {
                ...
                delta.pushKV("address", EncodeDestination(ScriptHash(CScriptID(uint160(hashBytes)))));
            } else if (out.scriptPubKey.IsPayToPublicKeyHash()) {
                std::vector<unsigned char> hashBytes(out.scriptPubKey.begin() + 3, out.scriptPubKey.begin() + 23);
                delta.pushKV("address", EncodeDestination(PKHash(CKeyID(uint160(hashBytes)))));
            } else {
                continue;
            }
                ...
                {RPCResult::Type::OBJ, "scriptPubKey", "",
                    {
                        ...
                        {RPCResult::Type::ARR, "addresses", "array of sumcoin addresses",
