...
*Setup instructions for a Gitian build of Sumcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Sumcoin
...
- [Building Sumcoin Core](#building-sumcoin-core)
...
Clone the git repositories for sumcoin and Gitian.
...
in the sumcoin repository under 'Fetch and create inputs' to install sources which require
...
To build Sumcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the sumcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sumcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/sumcoinlabs/sumcoin
...
For example:
```bash
URL=https://github.com/thrasher-/sumcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit sumcoin=${COMMIT} --url sumcoin=${URL} ../sumcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sumcoin=${COMMIT} --url sumcoin=${URL} ../sumcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sumcoin=${COMMIT} --url sumcoin=${URL} ../sumcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the sumcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../sumcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/sumcoin
SIGPATH=/some/root/path/sumcoin-detached-sigs

./bin/gbuild --url sumcoin=${BTCPATH},signature=${SIGPATH} ../sumcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sumcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sumcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sumcoin-osx-build.assert
```
...
[sumcoinlabs/gitian.sigs.sum](https://github.com/sumcoinlabs/gitian.sigs.sum/) repository, or if that's not possible create a pull
