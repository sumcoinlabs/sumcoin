Sumcoin Core version 0.13.2 is now available from:

  <https://download.sumcoin.info/sumcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a sumcoin
...
ECDSA signatures inside Sumcoin transactions now use validation using
...
Previous versions of Sumcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Sumcoin Core was to raise the default minimum
relay fee.

Sumcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Sumcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running sumcoind without having to do any manual configuration.
...
Sumcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Sumcoin Core will relay transactions with
...
entirely in Sumcoin Core 0.13, and it is at this time undecided whether the
...
Sumcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Sumcoin Core automatically creates a hidden service to listen on, without
manual configuration. Sumcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Sumcoin Core is listening, and
...
Sumcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Sumcoin
Core to use floating fees. By default, Sumcoin Core will use floating
fees.
...
(default: `0.0002` SUM/kB).

At all times, Sumcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SUM.
Furthermore, Sumcoin Core will never create transactions paying less than
...
`sumcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Restarting sumcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in sumcoin.conf
...
For example:
    ...
    $ src/sumcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Sumcoin Core code base has
...
- `sumcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `sumcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Sumcoin scripting system that in combination with
...
Sumcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Sumcoin Core.
...
The two phases are distinct in the Sumcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Sumcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Sumcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Sumcoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Sumcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Sumcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Sumcoin's current ledger (called the UTXO set),
  ...
  note that Sumcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Sumcoin users to individually opt-in to almost any change in the Sumcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Sumcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Sumcoin
client to Sumcoin Core 0.13.2 to signal for segwit support. When a
...
Since Sumcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `sumcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `sumcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
