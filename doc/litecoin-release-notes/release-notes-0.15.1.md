Sumcoin Core version *0.15.1* is now available from:

  <https://download.sumcoin.org/sumcoin-0.15.1/>
...
A number of changes to the way Sumcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 sigmas per kB (0.00001 SUM). This is to prepare for dropping the minimum transaction fee to 0.1 sigmas per kB (0.0001 SUM) in the next release.
