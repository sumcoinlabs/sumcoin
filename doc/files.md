...
`./`               | `sumcash.conf`       | Contains [configuration settings](sumcash-conf.md) for `sumcoind` or `sumcash-qt`; can be specified by `-conf` option
`./`               | `sumcoind.pid`       | Stores the process ID (PID) of `sumcoind` or `sumcash-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `sumcoind` or `sumcash-qt`; can be specified by `-debuglogfile` option
