...
3. All content of the data directory, except for `sumcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `sumcoin.conf`       | Contains [configuration settings](sumcoin-conf.md) for `sumcoind` or `sumcoin-qt`; can be specified by `-conf` option
`./`               | `sumcoind.pid`       | Stores the process ID (PID) of `sumcoind` or `sumcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `sumcoind` or `sumcoin-qt`; can be specified by `-debuglogfile` option
...
`sumcoin-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
