...
The data directory is the default location where the Sumcoin files are stored.
...
---------|--------------------
Linux    | `$HOME/.sumcash/`
macOS    | `$HOME/Library/Application Support/Sumcoin/`
Windows  | `%APPDATA%\Sumcoin\` <sup>[\[1\]](#note1)</sup>

2. The non-default data directory path can be specified by `-datadir` option.

3. All content of the data directory, except for `sumcash.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Sumcoin blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `sumcash.conf`       | Contains [configuration settings](sumcash-conf.md) for `sumcashd` or `sumcash-qt`; can be specified by `-conf` option
`./`               | `sumcashd.pid`       | Stores the process ID (PID) of `sumcashd` or `sumcash-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `sumcashd` or `sumcash-qt`; can be specified by `-debuglogfile` option
...
`sumcash-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by the Sumcoin:
